...
- **edsh**: The main binary with subcommands for different roles.
  - **Client (`edsh <EndpointID>`)**: Designed to be used as an SSH `ProxyCommand`. It bridges standard I/O to an `iroh` stream.
...
2. **Client**: Configured via `~/.ssh/config` using `ProxyCommand edsh <EndpointID>`.
